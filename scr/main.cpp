...
float fov = glm::radians(60.0f);//60�
float near = 0.1f;
float far = 50.0f;
...
std::map<int, Orbit> orbits; //Mapa que almacena los datos de las �rbitas de los objetos (con un vector tendria que meter una orbita por objeto en memoria)
...
//Rescalado para mantener lso tamaños
void resizeFunc(int width, int height)
{
...
{
	if(objIds.size() >= 10) {
		std::cout << "L�mite de 10 objetos alcanzado, no se puede cargar m�s." << std::endl;
		return false;
	}
